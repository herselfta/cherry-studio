...
export async function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  if (isMac) {
    ...
    ipcMain.handle(IpcChannel.App_MacCodeSignatureInfo, () => {
      return getMacCodeSignatureInfo()
    })
  ...
  // Relaunch app
  ipcMain.handle(IpcChannel.App_RelaunchApp, async (_, options?: Electron.RelaunchOptions) => {
    // Fix for .AppImage
  ...
  ipcMain.handle(IpcChannel.Backup_BackupMigrationToLocalDir, backupManager.backupMigrationToLocalDir.bind(backupManager))
  ...
  ipcMain.handle(IpcChannel.Backup_BackupMigrationToS3, backupManager.backupMigrationToS3.bind(backupManager))
  ...
  ipcMain.handle(IpcChannel.Backup_BackupMigrationToWebdav, backupManager.backupMigrationToWebdav.bind(backupManager))
  ipcMain.handle(IpcChannel.Backup_UploadTextToWebdav, backupManager.uploadTextToWebdav.bind(backupManager))
  ipcMain.handle(IpcChannel.Backup_DownloadTextFromWebdav, backupManager.downloadTextFromWebdav.bind(backupManager))
