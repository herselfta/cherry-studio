...
export async function registerIpc(
	...
	ipcMain.handle(
		...
		async (_, options?: Electron.RelaunchOptions) => {
			// Fix for .AppImage
	...
	ipcMain.handle(IpcChannel.App_ResetData, async () => {
		...
		await fs.promises
			.rm(getDataPath(), { recursive: true, force: true })
			.catch((e) => {
				logger.warn(
					"Failed to remove Data directory (will be cleaned up on restart)",
					e as Error,
				);
			});
	});
	...
	ipcMain.handle(
		IpcChannel.Backup_DeleteTempBackup,
		backupManager.deleteTempBackup.bind(backupManager),
	);
	...
	ipcMain.handle(
		IpcChannel.OpenClaw_RestartGateway,
		openClawService.restartGateway,
	);
