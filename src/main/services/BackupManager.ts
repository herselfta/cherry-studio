...
class BackupManager {
  ...
  /**
   * Ensure copied files are writable before archiving/cleanup without clobbering execute bits.
   */
  private async setWritableRecursive(targetPath: string): Promise<void> {
    try {
      ...
      await Promise.all(items.map((item) => this.setWritableRecursive(path.join(targetPath, item))))
    } catch (error) {
      logger.warn(`[BackupManager] Failed to make backup path writable: ${targetPath}`, error as Error)
    }
  }
