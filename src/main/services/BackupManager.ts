...
class BackupManager {
	 ...
	 */
	private isS3ConfigEqual(
		cachedConfig: typeof this.cachedS3ConnectionConfig,
		config: S3Config,
	): boolean {
	...
	private getS3Storage(config: S3Config): S3Storage {
		// 检查核心连接配置是否变更
		const configChanged = !this.isS3ConfigEqual(
	...
	async backup(
		...
		const onProgress = (processData: {
			...
			if (
				logStages.includes(processData.stage) ||
		...
		try {
			...
			const calculateTotals = async (dirPath: string) => {
				try {
					...
					// 仅在出错时记录日志
					logger.error(
						"[BackupManager] Error calculating totals:",
						error as Error,
					);
				}
			};
	...
	async backupToS3(
		...
		const filename =
			s3Config.fileName ||
			`cherry-studio.backup.${deviceName}.${timestamp}.zip`;
	...
	async listLocalBackupFiles(
		...
		try {
			...
			logger.error(
				"[BackupManager] List local backup files failed:",
				error as Error,
			);
			throw error;
		}
	...
	async deleteLocalBackupFile(
		...
		try {
			...
			logger.error(
				"[BackupManager] Delete local backup file failed:",
				error as Error,
			);
			throw error;
		}
	...
	async createLanTransferBackup(
		...
		logger.info(
			`[BackupManager] Created LAN transfer backup at: ${backupedFilePath}`,
		);
		return backupedFilePath;
	}
	 ...
	 */
	async deleteTempBackup(
		_: Electron.IpcMainInvokeEvent,
		filePath: string,
	): Promise<boolean> {
		try {
			// Security check: only allow deletion within temp directory
			const tempBase = path.normalize(
				path.join(app.getPath("temp"), "cherry-studio", "lan-transfer"),
			);
			...
			if (
				!resolvedPath.startsWith(tempBase + path.sep) &&
				resolvedPath !== tempBase
			) {
				logger.warn(
					`[BackupManager] Attempted to delete file outside temp directory: ${filePath}`,
				);
				return false;
			}
			...
			logger.error(
				"[BackupManager] Failed to delete temp backup:",
				error as Error,
			);
			return false;
		}
