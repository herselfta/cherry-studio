...
} from "@main/configs/SelectionConfig";
...
export class SelectionService {
	...
	public setZoomFactor = (zoomFactor: number) => {
		...
		this.toolbarWindow!.webContents.setZoomFactor(zoomFactor);
		this.resizeToolbarWindow();
	};

	private initConfig(): void {
		this.triggerMode =
			configManager.getSelectionAssistantTriggerMode() as TriggerMode;
		...
		configManager.subscribe(
			ConfigKeys.SelectionAssistantTriggerMode,
		...
		configManager.subscribe(
			ConfigKeys.SelectionAssistantRemeberWinSize,
		...
		configManager.subscribe(
			ConfigKeys.SelectionAssistantFilterMode,
			(filterMode: string) => {
		...
		configManager.subscribe(
			ConfigKeys.SelectionAssistantFilterList,
			(filterList: string[]) => {
	...
	private setHookGlobalFilterMode(mode: string, list: string[]): void {
		...
		if (
			!this.selectionHook.setGlobalFilterMode(
				modeMap[combinedMode],
				combinedList,
			)
		) {
	...
	public toggleEnabled(enabled: boolean | undefined = undefined): void {
		...
		storeSyncService.syncToRenderer(
			"selectionStore/setSelectionEnabled",
			newEnabled,
		);
	}
	...
	private showToolbarAtPosition(
		...
		const { x: posX, y: posY } = this.calculateToolbarPosition(
			point,
			orientation,
		);
	...
	private isSameLineWithRectPoint(
		startTop: Point,
		startBottom: Point,
		endTop: Point,
		endBottom: Point,
	...
	public processSelectTextByShortcut(): void {
		if (
			!this.selectionHook ||
			!this.started ||
			this.triggerMode !== TriggerMode.Shortcut
		)
			return;
	 ...
	 */
	private shouldProcessTextSelection(
	...
	private processTextSelection = (selectionData: TextSelectionData) => {
		...
		if (
			!selectionData.text ||
			(this.isToolbarAlive() && this.toolbarWindow!.isVisible())
		) {
			return;
		}
		...
		switch (selectionData.posLevel) {
			...
			case SelectionHook?.PositionLevel.MOUSE_DUAL:
				{
					const yDistance =
						selectionData.mousePosEnd.y - selectionData.mousePosStart.y;
					const xDistance =
						selectionData.mousePosEnd.x - selectionData.mousePosStart.x;
			...
			case SelectionHook?.PositionLevel.SEL_DETAILED:
				{
					...
					const isDoubleClick = this.isSamePoint(
	...
	private startHideByMouseKeyListener(): void {
		try {
			...
			this.logError(
				"Failed to start global mouse event listener:",
				error as Error,
			);
		}
	}
	...
	private stopHideByMouseKeyListener(): void {
		...
		try {
			...
			this.logError(
				"Failed to stop global mouse event listener:",
				error as Error,
			);
		}
	}
	...
	private handleKeyDownHide = (data: KeyboardEventData) => {
		...
		if (
			this.triggerMode === TriggerMode.Ctrlkey &&
			this.isCtrlkey(data.vkCode)
		) {
			return;
		}
	...
	private createPreloadedActionWindow(): BrowserWindow {
		const preloadedActionWindow = new BrowserWindow({
			width: this.isRemeberWinSize
				? this.lastActionWindowSize.width
				: this.ACTION_WINDOW_WIDTH,
			height: this.isRemeberWinSize
				? this.lastActionWindowSize.height
				: this.ACTION_WINDOW_HEIGHT,
			minWidth: 300,
			minHeight: 200,
			frame: false,
			transparent: true,
	...
	private showActionWindow(
		...
		if (!this.isFollowToolbar || !this.toolbarWindow) {
			const centerX = Math.round(
				workArea.x + (workArea.width - actionWindowWidth) / 2,
			);
			const centerY = Math.round(
				workArea.y + (workArea.height - actionWindowHeight) / 2,
			);
			...
			let posX = Math.round(
				toolbarBounds.x + (toolbarBounds.width - actionWindowWidth) / 2,
			);
	...
	public static registerIpcHandler(): void {
		...
		ipcMain.handle(
			IpcChannel.Selection_SetFilterList,
			(_, filterList: string[]) => {
		...
		ipcMain.handle(
			IpcChannel.Selection_ActionWindowPin,
			(event, isPinned: boolean) => {
				const actionWindow = BrowserWindow.fromWebContents(event.sender);
				if (actionWindow && !actionWindow.isDestroyed()) {
		...
		ipcMain.handle(
			...
			(event, deltaX: number, deltaY: number, direction: string) => {
				const actionWindow = BrowserWindow.fromWebContents(event.sender);
				if (actionWindow && !actionWindow.isDestroyed()) {
					selectionService?.resizeActionWindow(
						actionWindow,
						deltaX,
						deltaY,
						direction,
