...
const NutstoreSettings: FC = () => {
	...
	const [nutstoreUsername, setNutstoreUsername] = useState<string | undefined>(
		undefined,
	);
	const [nutstorePass, setNutstorePass] = useState<string | undefined>(
		undefined,
	);
	...
	const [syncInterval, setSyncInterval] =
		useState<number>(nutstoreSyncInterval);
	const [nutSkipBackupFile, setNutSkipBackupFile] = useState<boolean>(
		nutstoreSkipBackupFile,
	);
	...
	const handleClickPathChange = async () => {
		...
		const targetPath = await NutstorePathPopup.show({
			ls: async (target: string) => {
				...
				const items = await window.api.nutstore.getDirectoryContents(
					token,
					target,
				);
	...
	return (
		<SettingGroup theme={theme}>
			...
			<SettingRow>
				<SettingRowTitle>
					{isLogin
						? t("settings.data.nutstore.isLogin")
						: t("settings.data.nutstore.notLogin")}
				</SettingRowTitle>
				{isLogin ? (
			...
			{isLogin && (
				<>
					<SettingRow>
						<SettingRowTitle>
							{t("settings.data.nutstore.username")}
						</SettingRowTitle>
					...
					<SettingRow>
						<SettingRowTitle>
							{t("settings.data.nutstore.path.label")}
						</SettingRowTitle>
					...
					<SettingRow>
						<SettingRowTitle>
							{t("settings.data.auto_sync.label")}
						</SettingRowTitle>
						<Switch
			...
			<>
				<WebdavBackupModal
					...
					customLabels={{
						...
						filenamePlaceholder: t(
							"settings.data.nutstore.backup.modal.filename.placeholder",
						),
					}}
				/>
				...
				<WebdavBackupManager
					...
					customLabels={{
						restoreConfirmTitle: t(
							"settings.data.nutstore.restore.confirm.title",
						),
						restoreConfirmContent: t(
							"settings.data.nutstore.restore.confirm.content",
						),
