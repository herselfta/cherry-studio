...
export function LocalBackupManager({
	...
	const fetchBackupFiles = useCallback(async () => {
		...
		try {
			...
			window.toast.error(
				`${t("settings.data.local.backup.manager.fetch.error")}: ${error.message}`,
			);
		} finally {
			setLoading(false);
		}
	...
	const handleDeleteSelected = async () => {
		if (selectedRowKeys.length === 0) {
			message.warning(
				t("settings.data.local.backup.manager.select.files.delete"),
			);
			return;
		}
		...
		window.modal.confirm({
			...
			content: t("settings.data.local.backup.manager.delete.confirm.multiple", {
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.local.backup.manager.delete.success.multiple", {
							count: selectedRowKeys.length,
						}),
					);
					setSelectedRowKeys([]);
					...
					window.toast.error(
						`${t("settings.data.local.backup.manager.delete.error")}: ${error.message}`,
					);
				} finally {
					setDeleting(false);
				}
	...
	const handleDeleteSingle = async (fileName: string) => {
		...
		window.modal.confirm({
			title: t("settings.data.local.backup.manager.delete.confirm.title"),
			icon: <ExclamationCircleOutlined />,
			content: t("settings.data.local.backup.manager.delete.confirm.single", {
				fileName,
			}),
			okText: t("common.confirm"),
			cancelText: t("common.cancel"),
			...
			onOk: async () => {
				...
				try {
					...
					message.success(
						t("settings.data.local.backup.manager.delete.success.single"),
					);
					await fetchBackupFiles();
				} catch (error: any) {
					window.toast.error(
		...
		window.modal.confirm({
			...
			onOk: async () => {
				...
				try {
					...
					message.success(
						t("settings.data.local.backup.manager.restore.success"),
					);
					onClose(); // Close the modal
				} catch (error: any) {
					window.toast.error(
						`${t("settings.data.local.backup.manager.restore.error")}: ${error.message}`,
					);
				} finally {
					setRestoring(false);
				}
