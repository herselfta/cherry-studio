import { backupToS3, LEGACY_INTERNAL_BACKUP_FILE_NAME } from '@renderer/services/BackupService'
...
export function useS3RestoreModal({
  ...
  const showRestoreModal = useCallback(async () => {
    ...
    try {
      ...
      setBackupFiles(files.filter((file) => file.fileName !== LEGACY_INTERNAL_BACKUP_FILE_NAME))
    } catch (error: any) {
      window.toast.error(
