...
export function WebdavBackupManager({
	...
	const fetchBackupFiles = useCallback(async () => {
		...
		try {
			...
			const visibleFiles = files.filter(
				(file) => file.fileName !== LEGACY_INTERNAL_BACKUP_FILE_NAME,
			);
			...
			window.toast.error(
				`${t("settings.data.webdav.backup.manager.fetch.error")}: ${error.message}`,
			);
		} finally {
			setLoading(false);
		}
	...
	const handleDeleteSelected = async () => {
		if (selectedRowKeys.length === 0) {
			message.warning(
				t("settings.data.webdav.backup.manager.select.files.delete"),
			);
			return;
		}
		...
		window.modal.confirm({
			...
			content: t(
				"settings.data.webdav.backup.manager.delete.confirm.multiple",
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.webdav.backup.manager.delete.success.multiple", {
							count: selectedRowKeys.length,
						}),
					);
					setSelectedRowKeys([]);
					...
					window.toast.error(
						`${t("settings.data.webdav.backup.manager.delete.error")}: ${error.message}`,
					);
				} finally {
					setDeleting(false);
				}
	...
	const handleDeleteSingle = async (fileName: string) => {
		...
		window.modal.confirm({
			title: t("settings.data.webdav.backup.manager.delete.confirm.title"),
			icon: <ExclamationCircleOutlined />,
			content: t("settings.data.webdav.backup.manager.delete.confirm.single", {
				fileName,
			}),
			okText: t("common.confirm"),
			cancelText: t("common.cancel"),
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.webdav.backup.manager.delete.success.single"),
					);
					await fetchBackupFiles();
				} catch (error: any) {
					window.toast.error(
	...
	const handleRestore = async (fileName: string) => {
		if (!webdavHost) {
			window.toast.error(
				customLabels?.invalidConfigMessage || t("message.error.invalid.webdav"),
			);
			return;
		}
		...
		window.modal.confirm({
			title:
				customLabels?.restoreConfirmTitle ||
				t("settings.data.webdav.restore.confirm.title"),
			icon: <ExclamationCircleOutlined />,
			content:
				customLabels?.restoreConfirmContent ||
				t("settings.data.webdav.restore.confirm.content"),
			okText: t("common.confirm"),
			cancelText: t("common.cancel"),
			centered: true,
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.webdav.backup.manager.restore.success"),
					);
					onClose(); // 关闭模态框
				} catch (error: any) {
					window.toast.error(
						`${t("settings.data.webdav.backup.manager.restore.error")}: ${error.message}`,
					);
				} finally {
					setRestoring(false);
				}
