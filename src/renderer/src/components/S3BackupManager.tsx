...
export function S3BackupManager({
	visible,
	onClose,
	s3Config,
	restoreMethod,
}: S3BackupManagerProps) {
	...
	const fetchBackupFiles = useCallback(async () => {
		...
		try {
			...
			window.toast.error(
				t("settings.data.s3.manager.files.fetch.error", {
	...
	const handleDeleteSelected = async () => {
		...
		window.modal.confirm({
			...
			content: t("settings.data.s3.manager.delete.confirm.multiple", {
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.s3.manager.delete.success.multiple", {
							count: selectedRowKeys.length,
						}),
					);
					setSelectedRowKeys([]);
					...
					window.toast.error(
						t("settings.data.s3.manager.delete.error", {
							message: error.message,
						}),
					);
				} finally {
	...
	const handleDeleteSingle = async (fileName: string) => {
		...
		window.modal.confirm({
			title: t("settings.data.s3.manager.delete.confirm.title"),
			icon: <ExclamationCircleOutlined />,
			content: t("settings.data.s3.manager.delete.confirm.single", {
				fileName,
			}),
			okText: t("settings.data.s3.manager.delete.confirm.title"),
			cancelText: t("common.cancel"),
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.success(
						t("settings.data.s3.manager.delete.success.single"),
					);
					await fetchBackupFiles();
				} catch (error: any) {
					window.toast.error(
		...
		window.modal.confirm({
			...
			onOk: async () => {
				...
				try {
					...
					window.toast.error(
						t("settings.data.s3.restore.error", { message: error.message }),
					);
				} finally {
					setRestoring(false);
				}
	...
	const footerContent = (
		<Space align="center">
			...
			<Button
				...
				loading={deleting}
			>
				{t("settings.data.s3.manager.delete.selected", {
					count: selectedRowKeys.length,
				})}
			</Button>
