...
describe('BackupLocalStorage', () => {
  ...
  it('only snapshots cross-device localStorage keys', () => {
    ...
    expect(createBackupLocalStorageSnapshot()).toEqual({
      [BACKUP_MANUAL_SYNC_CONFIRM_PREFERENCES_KEY]: '{"webdav":false,"s3":true,"local":true,"nutstore":true}',
  ...
  it('restores cross-device localStorage keys and keeps schedule times device-local', () => {
    ...
    restoreBackupLocalStorageSnapshot(
      {
        [PERSISTED_REDUX_STATE_STORAGE_KEY]: 'persisted-redux',
        [BACKUP_MANUAL_SYNC_CONFIRM_PREFERENCES_KEY]: '{"webdav":false,"s3":true,"local":false,"nutstore":true}'
      },
      { removeMissingPortableKeys: true }
    )

    expect(localStorage.getItem(PERSISTED_REDUX_STATE_STORAGE_KEY)).toBe('persisted-redux')
    expect(JSON.parse(localStorage.getItem(MANUAL_SYNC_SCHEDULE_STORAGE_KEY) || '')).toEqual({
      webdav: {
        uploadTimes: [],
        restoreTimes: [],
        confirmBeforeRestore: false
