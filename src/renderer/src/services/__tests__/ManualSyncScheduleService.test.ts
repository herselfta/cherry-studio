...
describe("ManualSyncScheduleService", () => {
	...
	it("normalizes daily times and removes duplicates", () => {
		expect(sortManualSyncTimes(["9:05", "23:10", "09:05", "invalid"])).toEqual([
			"09:05",
			"23:10",
		]);
	...
	it("calculates the next daily occurrence across multiple times", () => {
		const sameDay = getNextManualSyncOccurrence(
			new Date(2026, 2, 12, 8, 10, 0),
			["08:30", "21:15"],
		);
		const nextDay = getNextManualSyncOccurrence(
			new Date(2026, 2, 12, 23, 59, 0),
			["08:30", "21:15"],
		);
	...
	it("picks the latest manual backup and skips the legacy internal sync snapshot", () => {
		expect(
			pickLatestManualBackup([
				{
