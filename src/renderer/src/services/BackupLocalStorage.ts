...
export const MANUAL_SYNC_SCHEDULE_STORAGE_KEY =
	"cherry-studio:manual-sync:schedules:v1";
export const BACKUP_MANUAL_SYNC_CONFIRM_PREFERENCES_KEY =
	"cherry-studio:backup:manual-sync-confirm:v1";
...
export const BACKUP_AWARE_LOCAL_STORAGE_KEYS = [
	"language",
	"memory_currentUserId",
] as const;

type BackupAwareLocalStorageKey =
	(typeof BACKUP_AWARE_LOCAL_STORAGE_KEYS)[number];
...
type ManualSyncScheduleMap = Record<
	ManualSyncProvider,
...
function createDefaultManualSyncScheduleMap(): ManualSyncScheduleMap {
	return MANUAL_SYNC_PROVIDERS.reduce<ManualSyncScheduleMap>(
		(result, provider) => {
			result[provider] = {
				uploadTimes: [],
				restoreTimes: [],
...
function parseManualSyncConfirmPreferences(
	value: unknown,
): ManualSyncConfirmPreferences | null {
	if (!value || typeof value !== "object") {
		return null;
	}
	...
	const preferences =
		MANUAL_SYNC_PROVIDERS.reduce<ManualSyncConfirmPreferences>(
			(result, provider) => {
				const providerValue = (value as Record<string, unknown>)[provider];
				...
				if (
					...
					typeof (providerValue as Record<string, unknown>)
						.confirmBeforeRestore === "boolean"
				) {
					hasPortablePreference = true;
					result[provider] = (
						providerValue as Record<string, boolean>
					).confirmBeforeRestore;
					return result;
				}
...
function getManualSyncConfirmPreferences(
	storage: Storage,
): ManualSyncConfirmPreferences {
	const currentSchedules = parseManualSyncScheduleMap(
		readSerializedJson(storage.getItem(MANUAL_SYNC_SCHEDULE_STORAGE_KEY)),
	);

	return MANUAL_SYNC_PROVIDERS.reduce<ManualSyncConfirmPreferences>(
		(result, provider) => {
...
function getBackupManualSyncConfirmPreferences(
...
function asStorageSnapshot(
	value: unknown,
): Record<string, unknown> | BackupLocalStorageSnapshot {
	return value && typeof value === "object"
		? (value as Record<string, unknown>)
		: {};
}

export function createBackupLocalStorageSnapshot(
	storage: Storage = localStorage,
): BackupLocalStorageSnapshot {
	const snapshot: BackupLocalStorageSnapshot = {};
	const persistedReduxState = storage.getItem(
	...
	snapshot[BACKUP_MANUAL_SYNC_CONFIRM_PREFERENCES_KEY] = JSON.stringify(
		getManualSyncConfirmPreferences(storage),
	);
...
export function restoreBackupLocalStorageSnapshot(
	...
	const manualSyncConfirmPreferences =
		getBackupManualSyncConfirmPreferences(snapshot);
	if (manualSyncConfirmPreferences) {
		const currentScheduleMap = parseManualSyncScheduleMap(
			readSerializedJson(storage.getItem(MANUAL_SYNC_SCHEDULE_STORAGE_KEY)),
		);
		for (const provider of MANUAL_SYNC_PROVIDERS) {
			currentScheduleMap[provider].confirmBeforeRestore =
				manualSyncConfirmPreferences[provider];
		}
		storage.setItem(
			MANUAL_SYNC_SCHEDULE_STORAGE_KEY,
			JSON.stringify(currentScheduleMap),
		);
	}
