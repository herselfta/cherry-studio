...
async function createNutstoreConfig(
...
async function cleanupOldBackups(
	...
	try {
		...
		if (!files || !Array.isArray(files)) {
			logger.warn(
				"[cleanupOldBackups] Failed to list nutstore directory contents",
			);
			return;
		}
		...
		if (backupFiles.length < maxBackups) {
			logger.info(
				`[cleanupOldBackups] No cleanup needed: ${backupFiles.length}/${maxBackups} backups`,
			);
			return;
		}
		...
		logger.info(
			`[cleanupOldBackups] Deleting ${filesToDelete.length} old backup files`,
		);
		...
		for (const file of filesToDelete) {
			try {
				...
				logger.error(
					`[cleanupOldBackups] Failed to delete ${file.fileName}:`,
					error as Error,
				);
			}
		}
		...
		if (deletedCount > 0) {
			logger.info(
				`[cleanupOldBackups] Successfully deleted ${deletedCount} old backups`,
			);
		}
	} catch (error) {
...
export async function backupToNutstore({
	...
	try {
		...
		logger.error(
			"[backupToNutstore] Failed to get device type:",
			error as Error,
		);
	}
	const timestamp = dayjs().format("YYYYMMDDHHmmss");
	const backupFileName =
		customFileName || `cherry-studio.${timestamp}.${deviceType}.zip`;
...
export async function startNutstoreAutoBackup() {
	...
	if (!nutstoreToken) {
		logger.warn(
			"[startNutstoreAutoBackup] Invalid nutstore token, nutstore auto backup disabled",
		);
		return;
	}
...
function scheduleNextBackup(
	...
	if (nutstoreSyncInterval <= 0) {
		logger.warn(
			"[Nutstore AutoBackup] Invalid backup interval, nutstore auto backup disabled",
		);
		stopNutstoreAutoBackup();
		return;
	}
	...
	switch (scheduleType) {
		case "fromLastSyncTime":
			timeUntilNextBackup = Math.max(
				1000,
				(nutstoreSyncState?.lastSyncTime || 0) + requiredInterval - Date.now(),
			);
			break;
		case "fromNow":
...
async function performAutoBackup() {
	if (isAutoSyncRunning || isManualBackupRunning) {
		logger.verbose(
			"[Nutstore AutoBackup] Backup already in progress, rescheduling",
		);
	...
	if (isStreamingInProgress()) {
		logger.info(
			"[Nutstore AutoBackup] Streaming in progress, deferring backup",
		);
		scheduleNextBackup("fromNow");
		return;
	}
	...
	while (retryCount < maxRetries) {
		try {
			logger.verbose(
				`[Nutstore AutoBackup] Starting automatic backup... (attempt ${retryCount + 1}/${maxRetries})`,
			);
			store.dispatch(
			...
			if (retryCount === maxRetries) {
				logger.error(
					"[Nutstore AutoBackup] Automatic backup failed after all retries:",
					error as Error,
				);
				store.dispatch(
					setNutstoreSyncState({
				...
				logger.warn(
					`[Nutstore AutoBackup] Failed, retry ${retryCount}/${maxRetries} after ${backoffDelay / 1000}s`,
				);
