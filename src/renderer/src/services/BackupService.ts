...
// 重试删除S3文件的辅助函数
async function deleteS3FileWithRetry(
	fileName: string,
	s3Config: S3Config,
	maxRetries = 3,
) {
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			logger.verbose(
				`Successfully deleted old backup file: ${fileName} (attempt ${attempt})`,
			);
			return true;
		} catch (error: any) {
			lastError = error;
			logger.warn(
				`Delete attempt ${attempt}/${maxRetries} failed for ${fileName}:`,
				error.message,
			);
	...
	logger.error(
		`Failed to delete old backup file after ${maxRetries} attempts: ${fileName}`,
		lastError,
	);
	return false;
}
...
export async function backup(skipBackupFile: boolean) {
	...
	if (selectFolder) {
		await window.api.backup.backup(
			filename,
			fileContnet,
			selectFolder,
			skipBackupFile,
...
export async function backupToWebdav({
	...
	const finalFileName = backupFileName.endsWith(".zip")
		? backupFileName
		: `${backupFileName}.zip`;
	...
	try {
			...
			if (webdavMaxBackups > 0) {
				try {
					...
					if (currentDeviceFiles.length > webdavMaxBackups) {
						...
						logger.verbose(
							`Cleaning up ${filesToDelete.length} old backup files`,
						);
		...
		showMessage && window.toast.error(i18n.t("message.backup.failed"));
		logger.error(
			"[Backup] backupToWebdav: Error uploading file to WebDAV:",
			error,
		);
		throw error;
	} finally {
...
export async function restoreFromWebdav(fileName?: string) {
	...
	try {
		...
		logger.error(
			"[Backup] restoreFromWebdav: Error downloading file from WebDAV:",
			error,
		);
		window.modal.error({
	...
	try {
		...
		logger.error(
			"[Backup] Error downloading file from WebDAV:",
			error as Error,
		);
		window.toast.error(i18n.t("error.backup.file_format"));
	}
...
export function startAutoBackup(immediate = false, type?: BackupType) {
	if (!type) {
		...
		const { webdavAutoSync, webdavHost, localBackupAutoSync, localBackupDir } =
			settings;
	...
	if (type === "webdav") {
		...
		if (!webdavAutoSync || !webdavHost) {
			logger.info(
				"[WebdavAutoBackup] Invalid backup settings, auto backup disabled",
			);
			return;
		}
		...
		if (!s3Settings?.autoSync || !s3Settings?.endpoint) {
			logger.verbose(
				"[S3AutoBackup] Invalid backup settings, auto backup disabled",
			);
			return;
		}
		...
		if (!localBackupAutoSync || !localBackupDir) {
			logger.verbose(
				"[LocalAutoBackup] Invalid backup settings, auto backup disabled",
			);
			return;
		}
	...
	function scheduleNextBackup(
		...
		if (!syncInterval || syncInterval <= 0) {
			logger.verbose(
				`${logPrefix} Invalid backup interval, auto backup disabled`,
			);
		...
		switch (scheduleType) {
			case "fromLastSyncTime":
				timeUntilNextBackup = Math.max(
					1000,
					(lastSyncTime || 0) + requiredInterval - Date.now(),
				);
				break;
			case "fromNow":
	...
	async function performAutoBackup(backupType: BackupType) {
		...
		while (retryCount < maxRetries) {
			try {
				logger.verbose(
					`${logPrefix} Starting automatic backup... (attempt ${retryCount + 1}/${maxRetries})`,
				);
				...
				if (retryCount === maxRetries) {
					logger.error(
						`${logPrefix} Automatic backup failed after all retries:`,
						error as Error,
					);
					...
					await window.modal.error({
						title: i18n.t("message.backup.failed"),
						content:
							`${logPrefix} ${new Date().toLocaleString()} ` + error.message,
					});
				} else {
					const backoffDelay = Math.pow(2, retryCount - 1) * 10000 - 3000;
					logger.warn(
						`${logPrefix} Failed, retry ${retryCount}/${maxRetries} after ${backoffDelay / 1000}s`,
					);
	...
	if (data.version === 1) {
		...
		for (const { key, value } of data.indexedDB) {
			if (key.startsWith("topic:")) {
				await db
					.table("topics")
					.add({ id: value.id, messages: value.messages });
			}
			if (key === "image://avatar") {
		...
		if (result) {
			...
			if (localBackupMaxBackups > 0) {
				try {
					...
					const files =
						await window.api.backup.listLocalBackupFiles(localBackupDir);
					...
					if (currentDeviceFiles.length > localBackupMaxBackups) {
						...
						for (const file of filesToDelete) {
							logger.verbose(
								`[LocalBackup] Deleting old backup: ${file.fileName}`,
							);
							await window.api.backup.deleteLocalBackupFile(
								file.fileName,
								localBackupDir,
							);
						}
					...
					logger.error(
						"[LocalBackup] Failed to clean up old backups:",
						error as Error,
					);
				}
			}
...
export async function restoreFromLocal(fileName: string) {
	try {
		...
		const localBackupDir = await window.api.resolvePath(localBackupDirSetting);
		const restoreData = await window.api.backup.restoreFromLocalBackup(
			fileName,
			localBackupDir,
		);
		const data = JSON.parse(restoreData);
