...
import { MANUAL_SYNC_SCHEDULE_STORAGE_KEY } from "./BackupLocalStorage";
import {
	backupToLocal,
	backupToS3,
	backupToWebdav,
...
export function loadManualSyncSchedule(
	provider: ManualSyncProvider,
): ManualSyncScheduleConfig {
	return loadManualSyncSchedules()[provider];
}

export function saveManualSyncSchedule(
	provider: ManualSyncProvider,
	config: ManualSyncScheduleConfig,
) {
...
export function loadManualSyncSchedules(): ManualSyncScheduleMap {
	...
	try {
		...
		return PROVIDERS.reduce<ManualSyncScheduleMap>(
			(result, provider) => {
				result[provider] = normalizeManualSyncScheduleConfig(
					parsed?.[provider],
				);
				return result;
			},
		...
		logger.warn(
			"Failed to parse manual sync schedules, resetting them",
			error as Error,
		);
		return { ...DEFAULT_MANUAL_SYNC_SCHEDULES };
	}
...
export function normalizeManualSyncScheduleConfig(
	...
	return {
		...
		confirmBeforeRestore:
			config?.confirmBeforeRestore ??
			DEFAULT_MANUAL_SYNC_SCHEDULE_CONFIG.confirmBeforeRestore,
	};
}
...
export function sortManualSyncTimes(times: string[]): string[] {
	const normalized = times
		.map((time) => normalizeManualSyncTime(time))
...
export function getNextManualSyncOccurrence(
	now: Date,
	times: string[],
): Date | null {
...
export function pickLatestManualBackup(files: BackupFile[]): BackupFile | null {
	return (
		files.find((file) => file.fileName !== LEGACY_INTERNAL_BACKUP_FILE_NAME) ??
		null
	);
}
...
function scheduleAllManualSyncTasks() {
	...
	for (const provider of PROVIDERS) {
		scheduleManualSyncTask(
			provider,
			"upload",
			scheduleMap[provider].uploadTimes,
		);
		scheduleManualSyncTask(
			provider,
			"restore",
			scheduleMap[provider].restoreTimes,
		);
	}
}

function scheduleManualSyncTask(
	provider: ManualSyncProvider,
	...
	logger.debug(
		`[ManualSync] Scheduling ${action} for ${provider} at ${nextRunAt.toLocaleString()}`,
	);
		...
		void enqueueManualSyncTask(async () => {
			...
			if (schedulerStarted) {
				scheduleManualSyncTask(
					provider,
					action,
					loadManualSyncSchedule(provider)[
						`${action}Times` as "uploadTimes" | "restoreTimes"
					],
				);
			}
		});
...
async function runManualSyncTask(
	provider: ManualSyncProvider,
	action: ManualSyncAction,
) {
	if (!isProviderConfigured(provider)) {
		logger.warn(
			`[ManualSync] Skip ${action} for ${provider}: provider is not configured`,
		);
		return;
	}
...
async function runScheduledRestore(provider: ManualSyncProvider) {
	...
	if (!latestBackup) {
		logger.warn(
			`[ManualSync] Skip restore for ${provider}: no manual backup file found`,
		);
		return;
	}
	...
	if (confirmBeforeRestore) {
		...
		if (!confirmed) {
			logger.info(
				`[ManualSync] Scheduled restore for ${provider} was cancelled by user`,
			);
			return;
		}
	}
...
async function getLatestManualBackup(
	provider: ManualSyncProvider,
	...
	if (provider === "local") {
		const localBackupDir = await window.api.resolvePath(
			store.getState().settings.localBackupDir,
		);
...
async function confirmScheduledRestore(
	provider: ManualSyncProvider,
	file: BackupFile,
) {
